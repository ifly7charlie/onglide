...
export function establishOffset() {
	...
	if( tOffset ) {
		...
		console.log( `Using Offset ${tOffset.toFixed(0)}, base ${tBase}` );
	}
...
export async function startDBReplay( db, options, location, getAssociation, packetcallback, gliders, m = undefined ) {
	...
	setInterval( async function() {
		...
		for( const point of rawpoints ) {
            ...
            processPacket( aprsPacket, getAssociation, packetcallback );
		}
	},1000);

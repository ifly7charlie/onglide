...
export async function scorePilot( className, compno, data ) {
	...
	let [ task, pilots ] = (await fetchTaskAndPilots( className, compno ? true : false ));
    ...
    // If we were just doing a refresh then do nothing more
    if( ! compno ) {
        return true;
    }
	...
	if( ! tracker ) {
        conolse.log(`no data for tracker ${compno} found`)
		return false;
	}
...
//		tracker.firstOldPoint = points.length-1;
	...
	for( let i = Math.min(points.length-1,Math.max(tracker.oldestMerge||0,tracker.firstOldPoint||0)); i >= 0; i-- ) {
		let p = points[i];
		if( ! p.ll ) { p.ll = new LatLong( p.lat, p.lng ); }
        if( ! p.geoJSON ) { p.geoJSON = point([p.lng,p.lat]) };
    ...
    if( ! data.state?.[compno] ) {
        console.log( `missing state ${compno}, not scoring` );
        return;
    }
    ...
    await findStart( tracker, data.state[compno], task, data.points[compno] );

    // Actually score the task
    switch( task.task.type ) {
    case 'A': // Assigned Area Task
        await scoreAssignedAreaTask( task, tracker, data.state[compno], data.points[compno] );
        //scoreAssignedAreaTask(task.legs, trackers['WO'], points['WO']);
        break;
    case 'S': // speed task
        await scoreSpeedTask( task, tracker, data.state[compno], data.points[compno] );
        break;
    case 'D': // distance handicapped task (needs to know the maximum handicap to score it properly)
        await scoreDistanceHandicapTask( task, tracker, data.state[compno], data.points[compno], _maxby(trackers,'handicap') );
		break;
	case 'E':
		await scoreEglideTask( task, tracker, data.state[compno], data.points[compno] );
		break;
    default:
	...
	if( task.rules.handicapped == 'N' || task.task.type == 'E' || task.task.type == 'D' ) {
		// And then we remove the handicapped speed when it's not relevant
		...
		_foreach( tracker.legs, (l) => delete l.handicapped );
	}
    ...
    if( ! tracker || ! tracker.compno ) {
        ...
        if( pilot.datafromscoring == 'Y' || pilot.finish == 'Y' ) {
            var copyKeys = [ 'start', 'utcstart', 'finish', 'utcfinish', 'dbstatus', 'statustext', 'utctime', 'datafromscoring', 'duration',
                             'hspeed', 'speed', 'hdistancedone', 'distancedone', 'scoredstatus' ];
...
export async function fetchTaskAndPilots(className,cache) {
	// Fetch the tasks, legs, competition rules etc.  Needed for scoring
    // try cache
    let { task, pilots } = (cache ? (kvs.get(className+'_task_pilots')) : null)||{task:undefined,pilots:undefined};
    if( !task || !pilots) {
		console.log( `Fetching task and raw pilots for ${className}, cache=${cache}`);

...
import LatLong from '../lib/LatLong.js';
...
import scoreAssignedAreaTask from '../lib/scoreassignedareatask.js'
import scoreSpeedTask from '../lib/scorespeedtask.js'
import scoreDistanceHandicapTask from '../lib/scoredistancehandicaptask.js'
...
export default async function scorePilot( className, compno, data ) {
    ...
    _foreach( pilots, (pilot,compno) => {
        ...
        data.trackers[compno].taskduration = task.task.durationsecs;
    });
	...
	for( let i = 0; i < tracker.oldestMerge||tracker.firstOldPoint||0; i++ ) {
		let p = points[i];
    ...
    findStart( tracker, data.state[compno], task.legs, data.points[compno] );
    ...
    if( tracker.scoredpoints && tracker.scoredpoints.length>1 ) {
        tracker.scoredGeoJSON = lineString(tracker.scoredpoints,{})
    }
    else {
        delete tracker.scoredpoints;
    }

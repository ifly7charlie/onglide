...
export default function scoreSpeedTask( task, tracker, state, points ) {
	...
	// Check for restart
	if( state.lastProcessedTime < tracker.utcstart+1 || t == 0 ) {
		...
		tracker.lasttp = 0;
		tracker.legs = { 0: { time: tracker.utcstart }};
    ...
    while( p >= 0 && t < legs.length ) {
        ...
        if( advancetp ) {
			...
			tracker.legs[t] = {
				leg: t, time: points[p].t,
				lat:round(legs[t].ll.dlat(),5), lng:round(legs[t].ll.dlong(),5),
				alt:points[p].a, agl:points[p].g,
				duration: (points[p].t-tracker.legs[t-1].time),
				distance: round(legs[t].length,1),
				distancedone: round(task.task.distance - r.remainingdistance,1),
				hdistancedone: round(state.htaskdistance - r.hremainingdistance,1),
			};
			tracker.legs[t] = { ...tracker.legs[t],
				legspeed: t > 0 ? round( legs[t].length / (tracker.legs[t].duration/3600), 1) : 0,
				taskspeed: round( tracker.legs[t].distancedone / ((points[p].t-tracker.utcstart)/3600), 1),
								htaskspeed: round( tracker.legs[t].hdistancedone / ((points[p].t-tracker.utcstart)/3600), 1),
				z: 1,
			}
    ...
    if( t == legs.length ) {
        ...
        const nextdist = round(closesttonext,1); // Math.round(Math.min(LatLong.distVincenty( points[0].ll, legs[t].ll),legs[t].length)*10)/10
		...
		tracker.legs[t] = {
			leg: t, time: points[0].t,
			lat:round(scoredTo.dlat(),5), lng:round(scoredTo.dlong(),5),
			alt:points[0].a, agl:points[0].g,
			distance: round(legs[t].length-nextdist,1),
			distancetonext: round(nextdist,1),
			distancedone: round(maxdistancedone,1),
			hdistancedone: round(hdistancedone,1),
			remainingdistance: tracker.remainingdistance = round(r.remainingdistance,1),
			hremainingdistance: tracker.hremainingdistance = round(r.hremainingdistance,1),
			grremaining: round((r.remainingdistance*1000)/(points[0].g),0),
            hgrremaining: round((r.hremainingdistance*1000)/(points[0].g),0),
			legspeed: t > 0 ? round( legs[t].distance / ((points[0].t-tracker.legs[t-1].time)/3600), 1) : 0,
			taskspeed: round( maxdistancedone / ((points[0].t-tracker.utcstart)/3600), 1),
			htaskspeed: round( hdistancedone / ((points[0].t-tracker.utcstart)/3600), 1),
			x: 1,
		}

		tracker.hgrremaining = tracker.legs[t].hgrremaining;
		tracker.grremaining = tracker.legs[t].grremaining;
	}

...
export default async function scoreSpeedTask( task, tracker, state, points ) {
...
//	console.log( `${tracker.compno}, start time:, ${tracker.start} (${tracker.utcstart}), ${points.length} points, ${(Date.now()/1000-points[0].t).toFixed(1)}s` );
	...
	if( (state.lastProcessedTime||0) < tracker.utcstart+1 || state.lastStartTime != tracker.utcstart ) {
		log( 'reset state', tracker.compno, state );
	...
	let hrStart = lastYield;
	let yields = 0;

	while( p >= 0 && t < legs.length ) {

        // Make sure we yield regularily
        if( process.hrtime(lastYield)[1] > 30*1e6 ) {
			await setTimeout(0);
...
//            await setTimeout(1);
	...
	const duration = process.hrtime(hrStart);
	console.log( `${tracker.compno}: elapsed: ${(duration[0] * 1000 + duration[1] / 1000000).toFixed(0)} ms, ${yields} yields`);
    ...
    if( t == legs.length ) {
        ...
        const scoredTo = LatLong.intermediatePoint(legs[lasttp-1].ll,legs[lasttp].ll,
                                                   (legs[lasttp].length)/6371,(legs[lasttp].length/legs[lasttp].length));
		log( 'scoredTo', legs[lasttp] );

...
export default function scoreSpeedTask( task, tracker, state, points ) {
    ...
    while( p >= 0 && t < legs.length ) {
        ...
        log( ` ${insector?'in sector':'not in sector'} (${t}) ${points[p].t - ptime} elapsed, distance ${curdist.toFixed(2)}, closest ${closesttonext.toFixed(2)}` );
		...
		// of finish area)
        if ( t !== finishLeg && state?.closestpointonsector?.geometry?.coordinates ) {
			...
			if( (points[p].t - ptime) > 20 ) {
				...
				const distanceNeeded =
					length( lineString( [ points[p].geoJSON.geometry.coordinates,
										  state.closestpointonsector.geometry.coordinates,
				...
				const possibleSpeed = (elapsedTime > 600 ? 160 : elapsedTime < 120 ? 330 : 200);
				...
				if( neededSpeed < possibleSpeed ) {
					log( `* dog leg ${t}, ${distanceNeeded.toFixed(1)} km needed, gap length ${elapsedTime} seconds` +
						 ` could have achieved distance in the time: ${neededSpeed.toFixed(1)} kph < ${possibleSpeed} kph` );
					advancetp = 1;
				}
				else {
					log( `- no dog log possible ${neededSpeed.toFixed(1)} kph over ${distanceNeeded.toFixed(1)} is too fast` );
				}
			}
			// Or are they are further away now, but the closest point was in the penalty area for the sector
			// this DOES NOT correctly place the change of TP
			else if( curdist > (closesttonext + 1) && closesttonext < 0.5 ) {
				log( `* penalty volume distance ${closesttonext.toFixed(1)} to tp ${t}, assuming next turnpoint` );
				// TODO: loop backwards to find closestpointonsector.properties.t in the point array and reset p to be
				//       that so we correctly split up the legs
				advancetp = 1;
			}
		}

...
export default function scoreSpeedTask( task, tracker, state, points ) {
	...
	let closesttonext = state?.closesttonext || 0; // how close to the next tp have we got, only used for last leg
    ...
    if( t == legs.length ) {
		...
		const r = remainingDistance( nextdist, t );
		...
		tracker.legs[t] = {
			...
			legspeed: t > 0 ? round( legs[t].length / ((points[0].t-tracker.legs[t-1].time)/3600), 1) : 0,

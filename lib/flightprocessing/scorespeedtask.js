...
export default function scoreSpeedTask( task, tracker, state, points ) {
    ...
    let t = tracker.state?.t || 1;
    let p = tracker.state?.p ? (tracker.state.p + (tracker.firstOldPoint||0)) : points.length-2;

	if( p < 0 || (tracker.firstOldPoint == undefined)) {
		log( "no new points" );
		return;
	}
    ...
    let distancedone = tracker.state?.distancedone || 0;
    let hdistancedone = tracker.state?.hdistancedone || 0;
    let maxdistancedone = tracker.state?.maxdistancedone || 0;

...
function addPoint(state_stack, state, point, lastpoint) {
    ...
    let current = {geoJSON: point.geoJSON, alt: point.a, time: point.t, lasttime: point.t, turncount: 0, distance: 0, heightgain: 0, heightloss: 0, packets: 0, maxdelay: 0, mixed: false, ws: emptyWs(point.t), direction: tdirection, points: [point.geoJSON]};
    ...
    if (stackEnd) {
        ...
        if (stackEnd.ws.cumulative < -361 || stackEnd.ws.cumulative > 361) {
            stackEnd.ws.history.push({min: _clonedeep(stackEnd.ws.min), max: _clonedeep(stackEnd.ws.max), cumulative: stackEnd.ws.cumulative, packets: stackEnd.ws.packets, endAltitude: point.a, startTime: twoffset(stackEnd.ws.time)});
...
function coalesceStack(source_state_stack) {
    function logState(current, action, dist) {
        log(`${current.time} (${twoffset(current.time)}): ${action} ${current.state} ${current.alt}m   ${current.lasttime - current.time}s  ${Math.round(current.turncount)} ${Math.round(current.distance * 10) / 10}km  +${current.heightgain - current.heightloss}m [${current.direction}] =${dist || ''}`);
    }

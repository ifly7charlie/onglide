...
export default function generateStatistics( tracker, state, points ) {
	...
	const binningTime = (points[0].t - (points[0].t%binningSize));
	const binUpdate = {
		speed: tracker.speed,
		hspeed: tracker.hspeed,
		distance: tracker.distance,
		hdistance: tracker.hdistance,
		winddirection: lastwinddirection,
		windspeed: lastwindspeed,
		altitude: state.stats.state.alt,
	}

	if( state?.binningstart != tracker.utcstart ) {
		state.binningstart = tracker.utcstart;
	}

	// If we have a record for this time then average new values into it, otherwise start a new record
	if( tracker.bytime?.[binningTime] ) {
		_assignwith( tracker.bytime[binningTime], binUpdate,	(a,b) => ((a||b)+b)/2 );
	}
	else {
		if( ! tracker.bytime ) {
			tracker.bytime = {};
		}
		tracker.bytime[binningTime] = binUpdate;
	}
		

	log( tracker.bytime[binningTime] );

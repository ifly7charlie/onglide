...
export function checkIsInTP( turnpoint, p, nearestPoint = undefined ) {
    ...
    if( turnpoint.quickSector && x < ( turnpoint.maxR + 0.5)) {
        return +x;
    }
...
export function distanceToTPPolygon( tp, point, nearestPoint ) {
	const r = nearestPointOnLine( lineString( tp.geoJSON.coordinates[0] ), point.geoJSON );
	if( nearestPoint ) {
		nearestPoint.geometry = r.geometry;
		nearestPoint.properties = { ...r.properties, t: point.t };
	}

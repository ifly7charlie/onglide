...
export default async function scoreAssignedAreaTask( task, tracker, state, points ) {
    ...
    function calcHandicap(dist,leg) {
        return ((100.0*dist)/Math.max(tracker.handicap+leg.Hi,25))
    }

    // We start from the start point not the track point
    if( t == 0 || t === undefined || ! state?.sectorpoints?.length ) {
	...
	let minNextDistPPrev = undefined;
    ...
    while( p > 0 && t < legs.length ) {
        ...
        const minDistance = 0.200; // 150m

        // Make sure we yield regularily
        if( process.hrtime(lastYield)[1] > 20*1e6 ) {
			await setTimeout(0);
			lastYield = process.hrtime()
        }

		// We don't want to add too many points to the dijkstras solver
		// as it gets very slow if there are so restrict it a bit
        ...
        if( Math.abs(_isinsector) < minNextDist ) {
            ...
            minNextDistP = p;
			minNextDistPPrev = pprev;
			log( `* ${_isinsector>=0?'in':''} sector ${t}, closest ${minNextDist.toFixed(2)}km [${_isinsector.toFixed(2)}km] @ t:${points[p].t}, p: ${p}, location: ${points[p].ll}`);
        }
		else {
			log( `* ${_isinsector>=0?'in':''} sector ${t}, closest ${minNextDist.toFixed(2)}km [${_isinsector.toFixed(2)}km] * t:${points[p].t}, p: ${p}, location: ${points[p].ll}`);
			log( `  ${minNextDistPPrev}, ${minNextDistP}, ${p} ` );
			if( _isinsector < 0 && minNextDistPPrev && points[p].t - points[minNextDistPPrev].t > 20 ) {
				const line = lineString([ points[minNextDistPPrev].geoJSON.geometry.coordinates,
										  points[minNextDistP].geoJSON.geometry.coordinates,
										  points[p].geoJSON.geometry.coordinates]);
				const elapsedTime = points[p].t - points[minNextDistPPrev].t;
				const totalDistance = length(line);
				const o = lineIntersect(line, legs[t].geoJSON );
				const speedKps = totalDistance / elapsedTime; // kps
				log( `  checking for intersection ${(speedKps/3600).toFixed(1)}kph, ${totalDistance.toFixed(2)}km, ${o.features.length} intersections` );
				if( o.features.length >= 2 ) {
					log( o.features[0].geometry, o.features[1].geometry )
					const tbase1 = tdistance( points[minNextDistPPrev].geoJSON,
											 o.features[0] ) * speedKps +
								   points[minNextDistPPrev].t;
					const tbase2 = tdistance( points[minNextDistPPrev].geoJSON,
											o.features[1] ) * speedKps +
								  points[minNextDistPPrev].t;
					const useP1 = state.pointsByTime[ tbase1 ] =  new LatLong( o.features[0].geometry.coordinates[1], o.features[0].geometry.coordinates[0] );
					const useP2 = state.pointsByTime[ tbase2 ] =  new LatLong( o.features[1].geometry.coordinates[1], o.features[1].geometry.coordinates[0] );
					log( ` ${tbase1}:useP1 ${useP1}, ${tbase2}:useP2 ${useP2}` );

					state.sectorpoints[t-1].forEach( function(previousPoint) {
						state.aatGraph.addLink( tbase1, previousPoint.t, 1000-LatLong.distHaversine( useP1, previousPoint.loc ));
					} );
					state.sectorpoints[t-1].forEach( function(previousPoint) {
						state.aatGraph.addLink( tbase2, previousPoint.t, 1000-LatLong.distHaversine( useP2, previousPoint.loc ));
					} );

					state.sectorpoints[t].push( {t:tbase1, loc:state.pointsByTime[tbase1]} );
					state.sectorpoints[t].push( {t:tbase2, loc:state.pointsByTime[tbase2]} );
					minNextDistP = minNextDistPPrev = undefined;
					minNextDistP = 999999999;
					wasinsector = 1;
				}
 			}
		}
...
//                state.pointsByTime[points[p].t] = useP;
            ...
            log(  "  [in AAT sector "+t+", point "+useP+", at "+points[p].t+(t != finishLeg?(' distN:'+ (1000-LatLong.distHaversine(useP,legs[t+1].ll))):'') + ", p "+p+"]");
            wasinsector = 1;
        }
    ...
    if( ! finish ) {
        ...
        // on circular sectors but not on wedges
			...
			const fpoint = points[minNextDistP ? minNextDistP : p];
            finish = fpoint.t;
			state.pointsByTime[finish] = fpoint.ll;
            state.sectorpoints[t-1].forEach( function(previousPoint) {
        ...
        dpoints = await tempGraph.shortestPath(tracker.utcstart, finish);
        ...
        // Calculate the longest path, doesn't include the start for some reason so we'll add it
        dpoints = await state.aatGraph.shortestPath(tracker.utcstart, finish);
        dpoints.push( ""+tracker.utcstart );
    }
    ...
    var leg = 0;
	var legStartTime = tracker.utcstart;
    ...
    dpoints.reverse().forEach( function(optimizedatstring) {
        ...
        if( leg == t && fakefinish ) { //&& tracker.stationary ) {
            log(`generating final point as incomplete task leg:[#${t}/${legs.length}, length ${legs[t].length}]`,
				`prevToTPCenter: ${LatLong.distHaversine(previousPoint,legs[t].ll) + (t==finishLeg?legs[t].maxR:0)}, minNextDistP:${minNextDistP}, pp: ${previousPoint} point:${point}`);
...
//            log("leg!=t"+leg);
		...
		let legStatus = { leg: leg, time: legStartTime, lat:round(point.dlat(),5), lng:round(point.dlong(),5), alt:point.a, agl:point.g };

		// The point is really start of next leg/finish of this leg so defer it to next time round
		legStartTime = at;
...
//            hdistancedone += (100.0*distance)/Math.max(tracker.handicap+legs[leg].Hi,25);
			const hleg = calcHandicap( distance, legs[leg] );
			hdistancedone += hleg;
			...
			legStatus.actual = {
				...
				distancetonext: round(minNextDist,1),
				distancedone: round(distdone,1),
				legspeed: round( distance / ((at-previousTime)/3600), 1),
				taskspeed: round( distdone / ((at-tracker.utcstart)/3600), 1),
			}
			legStatus.handicapped = {
				distance: round(hleg,1),
				distancetonext: round(calcHandicap(minNextDist,legs[leg]),1),
				distancedone: round(hdistancedone,1),
				legspeed: round( hleg / ((at-previousTime)/3600), 1),
				taskspeed: round( hdistancedone / ((at-tracker.utcstart)/3600), 1),
			}
    ...
    leg--;
    tracker.lasttp = leg;
	tracker.inturnpoint = wasinsector; 
    ...
    if( ! fakefinish ) {
        ...
        // Pick up remainder of current leg, don't adjust for finish ring as we want them home
        var distance = minNextDist; 
        ...
        tracker.remainingdistance = Math.round(remainingdistance*10)/10;
        tracker.hremainingdistance = Math.round(hremainingdistance*10)/10;
        if( points[0].g > 100 ) {
		...
		let legStatus = tracker.legs[leg];
        log( "legStatus" + legStatus );
        if( ! legStatus.actual ) { legStatus.actual = {} };
        if( ! legStatus.handicapped ) { legStatus.handicapped = {} };
		legStatus.actual.remainingdistance = round(remainingdistance,1);
		legStatus.actual.grremaining = points[0].g > 100 ? round((remainingdistance*1000)/points[0].g,1) : undefined;
		legStatus.handicapped.remainingdistance = round(hremainingdistance,1);
		legStatus.handicapped.grremaining = points[0].g > 100 ? round((hremainingdistance*1000)/points[0].g,1) : undefined;
    }


    // We can always calculate the speed and distance done
    // remember finish is last point in graph not the finish time
    var elapsed = (Math.max(finish,points[0].t) - tracker.utcstart)/3600;
    if( elapsed < 0 ) {

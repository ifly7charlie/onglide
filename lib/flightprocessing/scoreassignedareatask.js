...
//            log("leg!=t"+leg);
        ...
        if( previousPoint !== undefined && Math.round(distance*10) > 0 ) {
			...
			legStatus.distance = round(distance,1);
			legStatus.distancedone = round(distdone,1);
			legStatus.hdistancedone = round(hdistancedone,1);
			legStatus.distancetonext = round(minNextDist,1);
			
			legStatus.legspeed = round( distance / ((at-previousTime)/3600), 1);
			legStatus.taskspeed = round( distdone / ((at-tracker.utcstart)/3600), 1);		

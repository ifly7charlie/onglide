...
LatLong.distCosineLaw = function (p1, p2) {
    var R = 6371; // earth's mean radius in km
    var d = Math.acos(Math.sin(p1.lat) * Math.sin(p2.lat) + Math.cos(p1.lat) * Math.cos(p2.lat) * Math.cos(p2.lon - p1.lon)) * R;
    return d;
};
...
LatLong.midPoint = function (p1, p2) {
    ...
    lat3 = Math.atan2(Math.sin(p1.lat) + Math.sin(p2.lat), Math.sqrt((Math.cos(p1.lat) + Bx) * (Math.cos(p1.lat) + Bx) + By * By));
...
LatLong.intersection = function (p1, crs13, p2, crs23) {
    var crs12;
    var crs21;
    var dst12 = 2.0 * Math.asin(Math.sqrt(Math.pow(Math.sin((p1.lat - p2.lat) / 2.0), 2)) + Math.cos(p1.lat) * Math.cos(p2.lat) * Math.pow(Math.sin((p1.lon - p2.lon) / 2), 2));
...
LatLong.degToRad = function (brng) {
    ...
    switch (
        ...
        case 2: // interpret 2-part result as d/m
            ...
            break;
        default:
            var deg = parseFloat(brng);
            break; // otherwise decimal degrees
    }
    return (deg * Math.PI) / 180; // then convert to radians
};
...
LatLong.prototype.toString = function () {
    return this.dlat().toFixed(5) + ', ' + this.dlong().toFixed(5);
};

...
function adjustTask( task, state, handicap, highesthandicap ) {
	...
	// DOESN
	const [,fixedTps,fixedRadius] = task.contestday.notes.match(/TP([0-9,]*) radius: ([0-9]+)m/mi);
	...
	let hcapAdjustments = {};
	const [,handicappedTps] = task.contestday.notes.match(/TP([0-9,]+)\sRadius\sin\smeters/im);
	if( handicappedTps ) {
	...
	if( hcapAdjustments[handicap] ) {
		...
		console.warn( `no handicap adjustment found for ${handicap} in ${hcapAdjustments}` );
	}

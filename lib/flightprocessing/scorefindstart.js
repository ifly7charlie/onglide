...
export default function findStart( tracker, state, task, points )
{
	...
	if( rules.grandprixstart && task.nostart != '00:00:00' && task.nostartutc < (points[0]?.t||0)) {
		log( "grandprixstart" );
		tracker.utcstart = task.nostartutc;
	...
	if( task.nostart != '00:00:00' && tracker.utcstart < (task.nostartutc-10) ) {
	...
	if( (task.type == 'G' || rules?.grandprixstart == 'Y') && points[0].t > task.nostartutc ) {
		tracker.utcstart = task.nostartutc;
        tracker.start = timeToText(laststarttime);
        tracker.startFound = true;
        tracker.dbstatus = 'S';

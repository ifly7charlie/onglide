...
export function Details({units,pilot}) {
    ...
    */
	...
	const unitConvHeight = units ? 3.28084 : 1;
	const unitConvClimb = units ? 1.943844 : 1;

    const altitude =  pilot.altitude ? (<span>
                                            Altitude {Math.round(pilot.altitude*unitConvHeight)} {aglunit}  (AGL {Math.round(pilot.agl*unitConvHeight)} {aglunit})
                                        </span>) : null;
    ...
    const climb = (pilot.gainXsecond > 0 || pilot.lossXsecond > 0 ) ? (<><span>
                                                                             ...
                                                                             <Nbsp/><Icon type="circle-blank"/> {Math.round(pilot.average*unitConvClimb*10)/10} {climbunit}
                                                                         </span><br/></>) : null;
    ...
    const ognCoverage = uptodate ?
          (<span><Nbsp/><a href="#" style={{color:'white'}} title="In OGN Flarm coverage"><Icon type="check"/> {Math.round(pilot.delay)}s delay</a></span>) :
    ...
    return (
        <div className="details" style={{paddingTop:'5px'}}>
            {flag}<h6>{pilot.compno}:<b>{pilot.name}</b> {pilot.country}, {pilot.glidertype}, handicap {pilot.handicap}<div className={'pull-right'}>{pilot.follow?<Icon type="screenshot"/>:''}</div><br/>
...
function PilotStatusIcon({pilot}) {
    ...
    if( pilot.delay > 3600 && (pilot.dbstatus == 'G'|| pilot.dbstatus == 'S') && pilot.altitude ) {
        return (
...
export function PilotList({vc,pilots,selectedPilot,setSelectedCompno,options}) {
    ...
    updateSortKeys( mutatedPilotList, order, options.units );

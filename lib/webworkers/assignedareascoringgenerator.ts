...
import {Epoch, DistanceKM, AltitudeAMSL, AltitudeAgl, Compno, TaskStatus, EstimatedTurnType, Task, ScoredTaskDistanceStatus, TaskScoresGenerator, TaskStatusGenerator, BasePositionMessage, TaskLegStatus} from '../types';
...
export const assignedAreaScoringGenerator = function* (task: Task, taskStatusGenerator: TaskStatusGenerator, log?: Function): TaskScoresGenerator {
    ...
    function sumPath(path: BasePositionMessage[], startLeg: number = 0, saveLeg: Function = (_leg: number, _distance: DistanceKM, _point?: BasePositionMessage): void => {}) {
        ...
        for (const point of path) {
            console.log('sp:', leg, task.legs.length);
    ...
    let scoredStatus: ScoredTaskDistanceStatus;
    ...
    for (let current = iterator.next(); !current.done; current = iterator.next()) {
        ...
        if (taskStatus.startFound) {
            ...
            if (!taskStatus.utcFinish) {
                ...
                const fakePoint = {t: 0, lat: task.legs[taskStatus.currentLeg].nlat, lng: task.legs[taskStatus.currentLeg].nlng};
                const currentPoint = taskStatus.inSector ? taskStatus.closestToNextSectorPoint || taskStatus.lastProcessedPoint : fakePoint;

                const aatPreviousLeg = aatLegStatus[taskStatus.currentLeg - 1];
                if (aatPreviousLeg) {
                    for (const ppoint of aatPreviousLeg.convexHull) {
                        tempGraph.addLink(currentPoint, ppoint, distHaversine(currentPoint, ppoint));
                    }
                }
                ...
                scoredPoints = tempGraph.findPath(startPoint, currentPoint);
                ...
                for (let legno = maxLegStart; legno < finishLeg; legno++) {
                    ...
                    //                    console.log(legno, 'thisLegPoints:', JSON.stringify(thisLegPoints));
                    for (const sectorPoint of thisLegPoints) {
                        ...
                        for (const pPoint of previousMinPoints) {
                            ...
                            minGraph.addLink(pPoint, sectorPoint, (pPoint === fakeMinStart ? 0 : distHaversine(pPoint, sectorPoint)) as DistanceKM);
                        }
                    }
                    previousMinPoints = previousMaxPoints = thisLegPoints; // after first point they are the same
                }

...
function PilotImage(props) {
	...
	if( props.image ) {
        return <div className="ih" style={{backgroundImage: `url(//www.gravatar.com/avatar/${props.image}?d=robohash)`}}>
                   {props.country !== ''&& <div className="icountry">{isoCountryCodeToFlagEmoji(props.country)}</div>}
               </div>
	}
    if( props.country !== '' ) {
...
export function Details({units,pilot,tz,handicapped}) {
	...
	const hasHandicappedResults = pilot.legs[1] && pilot.legs[1].handicapped;

    const speed = <>Speed:
					  {pilot.hspeed && hasHandicappedResults ? <>{pilot.hspeed.toFixed(1)} kph hcap</> : null}
					  {pilot.speed.toFixed(1)} kph
				  </>;
	...
	if( pilot.legs ) {
		...
		function legIcon(legno,leg) {
			if( pilot.inturnpoint && (firstIncompleteLeg == legno || firstIncompleteLeg == (legno+1))) {
			...
			return (firstIncompleteLeg == legno) 
				? <Icon type='plane' tooltip="current leg"/> : <Icon type='check' tooltip="leg completed"/>
		};

		const accessor = viewOptions.hcapped ? (l) => l.handicapped : (l) => l.actual;
				
		legs = <>
				   <ButtonGroup toggle name="taskleg" role="group" aria-label="task or leg" value={viewOptions.task} className={"smallbuttons goleft"}>
				   ...
				   <table className="legs">
					   <thead>
						   <tr>
							   <td></td>
							   { _map( pilot.legs, (x) => x.leg > 0 ? <td>Leg {x.leg} {legIcon(x.leg,x)}</td> : null )}
						   </tr>
					   </thead>
					   <tbody>
						   <tr>
							   <td>Leg Start</td>
							   { _map( pilot.legs, (x) => x.leg > 0 ? <td>{formatTime(x.time,tz)[0]}</td> : null )}
						   </tr>
						   {(! viewOptions.task) ?
							<>
								<tr>
									<td>Leg Start Altitude</td>
									{ _map( pilot.legs, (x) => x.leg > 0 ? <td>{RoundNumber(x.agl)}</td> : null )}
								</tr>
								<tr>
									<td>Leg Speed</td>
					 ...
					 <>
						 <br clear="both"/>
						 {pilot.wind?.speed ?
						  <>Recent Wind {pilot.wind.speed} kph @ {pilot.wind.direction}</> : null}
						 <br/>
					 </>
				   }
			   </>;
    ...
    return (
        <>
			<span className="options">
				<a title="Show Radar" href="#" onClick={()=>props.setOptions('radar')}><Icon type="umbrella"/></a>
				<a title="Show Wind Shading" href="#" onClick={()=>props.setOptions('windshade')}><Icon type="magic"/></a>
			</span>
			<span className="sorting">
				...
				<a title="Show Distance Done" href="#" onClick={()=>props.setSort('distance')}><Icon type="signout "/>&nbsp;</a>
				<a title="Show Distance Remaining" href="#" onClick={()=>props.setSort('remaining')}><Icon type="signin "/>&nbsp;</a>
...
export function PilotList({vc,pilots,selectedPilot,setSelectedCompno,options,setOptions,now,tz}) {
    ...
    updateSortKeys( mutatedPilotList, order, options.units, now, tz );
    ...
    return (
        <>
            <Sorting setSort={(o)=>{setOrder(nextSortOrder(o,order))}} sortDescription={getSortDescription(order)}
					 setOptions={(o)=>{setOptions({...options,...o})}} options={options}

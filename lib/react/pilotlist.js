...
export function Details({units,pilot}) {
    ...
    const distance = (<>
                          <Optional b="Completed" v={pilot.hdistancedone} e=" km hcap"/>
                          <Optional b=", " v={pilot.distancedone||pilot.distance} e=" km "/>
                          {pilot.remaining? <><br/><Optional b="Remaining" v={pilot.remaining} e="km actual"/></>:null}
					  </>);
	...
	if( pilot.legs ) {
		...
		legs = <>
				   <table className="legs">
					   <thead>
						   <td>Tracked Leg</td>
						   { _map( pilot.legs, (x) => x.leg > 0 ? <td>{x.leg}</td> : null )}
					   </thead>
					   <tr>
						   <td>Leg Speed</td>
						   { _map( pilot.legs, (x) => x.leg > 0 ? <td>{x.legspeed}</td> : null )}
					   </tr>
					   <tr>
						   <td>Task Speed</td>
						   { _map( pilot.legs, (x) => x.leg > 0 ? <td>{x.taskspeed||''}</td> : null )}
					   </tr>
					   <tr>
						   <td>Leg Distance</td>
						   { _map( pilot.legs, (x) => x.leg > 0 ? <td>{x.distance||''}</td> : null )}
					   </tr>
					   <tr>
						   <td>Task Distance</td>
						   { _map( pilot.legs, (x) => x.leg > 0 ? <td>{x.distancedone||''}</td> : null )}
					   </tr>
				   </table>
			   </>;
	}
	...
	let times = null;
	if( pilot.start ) {
		times = 'Start '+pilot.start;
		if( pilot.duration ) {
			times += ' +'+pilot.duration;
		}
		if( pilot.finish) {
			times += ' Finish ' + pilot.finish;
		}
	}
    ...
    return (
        <div className="details" style={{paddingTop:'5px'}}>
            {flag}<h6>{pilot.compno}:<b>{pilot.name}</b> {pilot.glidertype} <div className={'pull-right'}>{pilot.follow?<Icon type="screenshot"/>:''}</div><br/>

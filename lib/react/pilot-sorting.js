...
export function updateSortKeys(trackers,sortKey,units,now,tz) {
    ...
    function updatePilotSortKey(tracker,sortKey,units,now,tz) {
        ...
        case 'start':
            if( tracker.utcstart ) {
				const stime = formatTime( tracker.utcstart, tz );
                displayAs = stime.substr(0,5);
                suffix = stime.substr(5,3);
            }
            newKey = tracker.utcstart;
            break;
        case 'finish':
				if( tracker.utcfinish ) {
					const ftime = formatTime( tracker.utcfinish, tz );
					displayAs = ftime.substr(0,5);
					suffix = ftime.substr(5,3);
				}

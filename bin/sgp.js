...
const oz_types = { 'Turnpoint': 'symmetrical',
                   'Finish':  'pp',
                   'Start':  'np' }
...
async function SGP(deep = false) {
    ...
    let keys = (await mysql_db.query(escape`
              SELECT *
                FROM scoringsource where type='sgp'`))[0];

    if( ! keys?.url ) {
        console.log( 'no SGP url configured' );
        return {
...
//		}
		...
		t.query( escape `
              INSERT INTO tracker ( class, compno, type, trackerid ) 
                  VALUES ( 'sgp', ${pilot.competitionId.substring(0,4)}, 'flarm', ${flarmid} )
...
async function update_task (task) {
    ...
    const hash = crypto.createHash('sha256').update(JSON.stringify(task)).digest('base64');
        ...
        .query( (r) => {
            ...
            for ( const tp of task.turnpoints ) {
                ...
                values = values.concat( [
                    'sgp', convert_to_mysql_datetime(date), taskid, i,
    ...
    // Update the last date for results
        .query( escape`UPDATE compstatus SET resultsdatecode = GREATEST(todcode(${convert_to_mysql_datetime(date)}),COALESCE(resultsdatecode,todcode(${convert_to_mysql_datetime(date)})))
                       WHERE class=${classid}`)

...
async function doIt() {
    ...
    for (const db of databases) {
        ...
        if (keys.type != '') {
            if (stop) {
                function n() {
                    pm2.delete(domain + '_next', () => process.exit(1));
                }
                function n2() {
                    pm2.delete(domain + '_ogn', n);
                }
                pm2.delete(domain + '_scoring', n2);
            } else {
                console.log('starting');
                if (!fe) {
                    const scoringScript = {soaringspotkey: 'dist/bin/soaringspot.js', rst: 'dist/bin/rst.js', sgp: 'dist/bin/sgp.js', soaringspotscrape: 'dist/bin/ssscrape.js'}[keys.type];
                ...
                function startNext() {
                    ...
                    if (restart) {
                        pm2.restart(domain + '_next', args, () => {
                            console.log('next restarted');
                            process.exit();
                        });
                    } else {

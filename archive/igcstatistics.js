import {bearing as tBearing, distance as tDistance} from '@turf/turf';
...
const binningSize = 120; // used to confirm the size for each time bin
...
function printStateStack(stack, log) {
    stack.forEach((now, index) => {
        ...
        log(
            `${now.state}  ${twoffset(now.time)} ${now.time}-${now.lasttime} ${now.alt}m   ${elapsed}s  ${now.turncount.toFixed(0)} ${now.distance.toFixed(2)}km vs ${tDistance(
                now.geoJSON,
                stack[index + 1]?.geoJSON || now.geoJSON,
                {units: 'kilometers'}
            ).toFixed(2)}km  +${now.heightgain - now.heightloss}m (${((now.heightgain - now.heightloss) / elapsed).toFixed(1)} m/s) [td:${now.direction}] ${(now.maxdelay / elapsed).toFixed(1)}% ${(
                elapsed / now.packets
            ).toFixed(0)}sec/pkts`
        );
        if (now?.wind?.speed) {
...
export default function generateStatistics(tracker, state, points) {
    ...
    if (tracker.compno == (process.env.NEXT_PUBLIC_COMPNO || '')) {
        log = function () {
            console.log(tracker.compno, ...arguments);
        };
    } else {
        log = function () {};
    }
    ...
    let lastwinddirection = undefined;
    let lastwindspeed = undefined;
    ...
    stack.forEach((now, index) => {
        ...
        if (now.state == 'thermal') {
            ...
            now.ws.history.forEach((circle) => {
                ...
                const [bisector, base] = absAngleDifference > 180 ? [(360 - absAngleDifference) / 2, circle.min.angle] : [absAngleDifference / 2, maxAngleInverted];
                ...
                if (quality >= 3.5 && quality <= 5) {
                    lastwindspeed = now.wind.speed = ((now.wind?.speed || windSpeed) + windSpeed) / 2;
                    lastwinddirection = now.wind.direction = ((now.wind?.direction || windAngle) + windAngle) / 2;
                }
            });
        }
    });
    ...
    let lastwind = {};
    stack.forEach((now, index) => {
        if (now.wind && now.wind.speed && now.wind.direction) {
            lastwind = now.wind;
        }
        tracker.stats.push({
            start: now.time,
            ...
            state: now.state,
            wind: lastwind,
            turncount: Math.floor(now.turncount),
            distance: Math.round(now.distance * 10) / 10,
            achievedDistance: Math.round(tDistance(now.geoJSON, stack[index + 1]?.geoJSON || now.geoJSON, {units: 'kilometers'}) * 10) / 10,
            delta: Math.round(now.heightgain - now.heightloss),
            avgDelta: Math.round(((now.heightgain - now.heightloss) / (now.lasttime - now.time)) * 10) / 10,
            direction: now.direction,
            heightgain: now.heightgain,
            heightlos: now.heightloss
        });
    });

    if (lastwind) {
        tracker.wind = lastwind;
    }

    console.log(tracker.compno, lastwind);
...
function addPoint(state_stack, state, point, lastpoint) {
    ...
    function bAvg(bearing) {
        ...
        state.pa = ((state.b || bearing) + bearing) / 2;
        state.b = bearing;
        return state.pa;
    }
    ...
    if (state.mode == 'thermal') {
        if (timedif > 5 && timedif < 20 && Math.round((((state.pa || 0) * timedif + state.previousbearing) % 360) / 10) == Math.round(bearing / 10)) {
    ...
    if (stackEnd) {
        ...
        if (stackEnd.ws.cumulative < -361 || stackEnd.ws.cumulative > 361) {
            stackEnd.ws.history.push({
                min: _clonedeep(stackEnd.ws.min),
                max: _clonedeep(stackEnd.ws.max),
                cumulative: stackEnd.ws.cumulative,
                packets: stackEnd.ws.packets,
                endAltitude: point.a,
                startTime: twoffset(stackEnd.ws.time)
            });
...
function coalesceStack(source_state_stack) {
    function logState(current, action, dist) {
        log(
            `${current.time} (${twoffset(current.time)}): ${action} ${current.state} ${current.alt}m   ${current.lasttime - current.time}s  ${Math.round(current.turncount)} ${Math.round(current.distance * 10) / 10}km  +${
                current.heightgain - current.heightloss
            }m [${current.direction}] =${dist || ''}`
        );
    }
    ...
    do {
        ...
        function merge(current, nxt, i, dist) {
            ...
            adjustments = true;
            log(
                `${current.time} (${twoffset(current.time)}): push ${current.state} forwards into ${nxt.state} bearing: ${current.turncount.toFixed(0)}=>${nxt.turncount.toFixed(0)} (d:${current.distance.toFixed(
                    1
                )}km =>${nxt.distance.toFixed(1)}km), ${current.direction}/${nxt.direction} =${dist || ''}`
            );
        ...
        for (let i = 0; i < state_stack.length - 1; ) {
            ...
            if (nxt.state == 'gap') {
                ...
                if (nxt.state == current.state || (elapsed < MINSTRAIGHTTIME[tolerance] && dist < MINSTRAIGHTDISTANCE[tolerance])) {
                    merge(current, nxt, i, dist);
                    if (i > 1) {
                        // we should re-evaluate the previous state, why only for straights??
                        i--;
                    }
                    continue;
                }

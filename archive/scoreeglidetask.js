...
export default async function scoreEglideTask(task, tracker, state, points, highesthandicap) {
    ...
    newTask.legs.forEach((leg) => {
        sectorGeoJSON(newTask.legs, leg.legno);
    });
...
function adjustTask(task, state, handicap, highesthandicap) {
    ...
    // DOESN
    const [, fixedTps, fixedRadius] = task.contestday.notes.match(/TP([0-9,]*) radius: ([0-9]+)m/im) || [undefined, undefined];

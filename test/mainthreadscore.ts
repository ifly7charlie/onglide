    ...
    for (const c of classes) {
        ...
        if (!channel.scoring && updatedTask) {
            ...
            if (process.env.REPLAY && !channel.replay) {
                console.log('replay track points');
        ...
        if (!_isEqual(channel.task || {}, updatedTask || {})) {
            console.log(`new task for ${c.class}: changed from ${channel.task?.details?.taskid || 'none'} to ${updatedTask?.details?.taskid || 'none'} [${channel.datecode}]`);
    ...
    */
    ...
    for (const gn in gliders) {
        ...
        if (!glider.scoringConfigured) {
            channel.scoring.setInitialTrack(glider.compno as Compno, glider.handicap, []);
            glider.scoringConfigured = true;
            if (!glider.deck) {

...
export default async function taskHandler(req, res) {
    ...
    if (points && points.length > 0) {
        marker.features = {
            type: 'Feature',
            properties: {i: 'dot', c: compno, v: Date.now() / 1000 - points[0].t > historyLength ? 'red' : 'blue', x: points[0].a + 'm (' + points[0].g + 'm agl)', t: points[0].t},
    ...
    res.status(200).json({track: trackJSON, marker: marker});
}

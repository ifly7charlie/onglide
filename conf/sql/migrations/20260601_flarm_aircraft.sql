...
--
--   flarm_aircraft         — one row per (flarmid, compid): glider key, greg,
--                            country, comp number, ICAO-permanence flag, the
--                            physical-track match_score (confidence the comp
--                            produced), observation count and first/last seen.
--   flarm_pilot            — one row per (flarmid, pilot_key, compid): club
--                            hash, FAI hash (the FAI id resolves to a named
--                            pilot, so it is hashed like a name), match_score,
--                            observations. A club glider flown by N pilots
--                            yields N pilot clues; one per comp each flew it.
--   flarm_pilot_nametoken  — HMAC name tokens per clue (comp-independent —
--                            name-derived); idx_token serves the part-2 reverse
--                            lookup (find flarmids by name).
--
-- compid scopes provenance so scoring can EXCLUDE the current competition
-- (within-comp continuity is the separate trackerhistory `prior`), and
-- match_score lets historical evidence be scaled by how confident that comp's
-- match actually was. match_score is the physical-track subscore only
-- (delta/distance/presence/pre-launch) — never the prior/identity-inflated
-- total — so evidence can't feed back on itself across comps.
--
